...
// Statuses considered "active" — anything not yet resolved
const ACTIVE_STATUSES = ['pending', 'dispatched', 'enroute', 'on_scene', 'transporting', 'at_hospital'];
...
export default function Tracking() {
  ...
  useEffect(() => {
    if (id) {
      // ── Direct link: /tracking/:id ──────────────────────────
      emergencyService.getById(id)
        .then(({ data }) => {
          setEmergency(data.emergency);
          setLiveStatus(data.emergency.status);
      ...
      // ── No id: load the user's latest active emergency ──────
      emergencyService.getMyEmergencies({ limit: 5 })
        .then(({ data }) => {
          const emergencies = data.emergencies || [];

          // Prefer an in-progress emergency first, then fall back to the most recent
          const active = emergencies.find(e => ACTIVE_STATUSES.includes(e.status));
          const latest = active || emergencies[0] || null;

          if (latest) {
            setEmergency(latest);
            setLiveStatus(latest.status);
            joinEmergencyRoom(latest._id);
          }
        })
        .catch(() => {})
        .finally(() => setLoading(false));
  ...
  if (loading) return (
    <DashboardLayout title="Live Tracking">
      <div className="flex justify-center py-20">
        <Loader size="lg" text="Loading emergency..." />
      </div>
    </DashboardLayout>
  );
    ...
    <DashboardLayout title="Live Tracking">
      {!emergency ? (
        ...
        <div className="max-w-xl mx-auto text-center py-20 space-y-6">
          ...
          <p className="text-ems-muted">You have no active or recent emergency to track right now.</p>
          <Link to="/emergency" className="btn-emergency inline-block py-4 px-8">
            Request Emergency →
          </Link>
        </div>
      ) : (
        ...
        <div className="max-w-4xl mx-auto space-y-6">
          ...
          <div className="ems-card border-emergency-red/30 bg-emergency-red/5">
            <div className="flex items-start justify-between mb-4">
              <div>
                ...
                <h2 className="text-2xl font-display text-ems-white">
                  {typeInfo?.label || emergency?.type}
                </h2>
                <p className="text-ems-muted text-sm">{emergency?.emergencyId}</p>
              </div>
              <div className="text-right">
                <div className="text-ems-muted text-xs">Reported</div>
            ...
            {emergency?.emt && (
              <div className="flex items-center gap-4 p-4 bg-ems-dark rounded-xl">
                ...
                <div className="flex-1">
                  <p className="text-ems-white font-semibold">
                    {emergency.emt.firstName} {emergency.emt.lastName}
                  </p>
                  <p className="text-ems-muted text-sm">
                    KMPDC-Certified EMT · {emergency.ambulance?.type || 'Ambulance'}
                  </p>
                </div>
                <a
                  href={`tel:${emergency.emt.phone}`}
                  className="btn-emergency py-2 px-4 flex items-center gap-2 text-sm"
                >
            ...
            {!emergency?.emt && ACTIVE_STATUSES.includes(currentStatus) && (
              <div className="flex items-center gap-3 p-4 bg-ems-dark rounded-xl">
                <div className="w-3 h-3 bg-emergency-red rounded-full animate-pulse flex-shrink-0" />
                <p className="text-ems-muted text-sm">
                  Dispatch is locating the nearest available unit — hold tight.
                </p>
              </div>
            )}
          </div>
                ...
                return (
                  <div key={s} className="flex items-start gap-4">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5
                      ...
                      : active ? 'bg-emergency-red'
                      :          'bg-ems-dark border border-ems-border'}`}
                    >
                      {done
                    ...
                    <div className={`flex-1 pb-4 border-b border-ems-border last:border-0
                      ${active ? 'opacity-100' : done ? 'opacity-70' : 'opacity-30'}`}
                    >
                      <p className={`font-medium text-sm ${active ? 'text-emergency-red' : 'text-ems-white'}`}>
                        {statusLabels[s]}
                      </p>
          ...
          <div className="grid sm:grid-cols-2 gap-5">
            {emergency?.patientLocation && (
              <div className="ems-card">
                <h3 className="text-ems-white font-semibold mb-4 flex items-center gap-2">
                  <FiMapPin size={15} className="text-emergency-red" /> Patient Location
                </h3>
                <div className="space-y-2 text-sm">
                  <p className="text-ems-muted">
                    {emergency.patientLocation.address || 'Address not provided'}
                  </p>
            ...
            {emergency?.hospital && (
              <div className="ems-card">
                <h3 className="text-ems-white font-semibold mb-4 flex items-center gap-2">
                  <FiActivity size={15} className="text-emergency-red" /> Destination Hospital
                </h3>
                ...
                {emergency.hospital.phone && (
                  <a
                    ...
                    className="flex items-center gap-2 text-emergency-red text-sm mt-2 hover:underline"
                  >
          ...
          <div className="ems-card border-emergency-red/20 bg-emergency-red/5 text-center">
            <p className="text-ems-muted text-sm mb-3">Need immediate assistance?</p>
