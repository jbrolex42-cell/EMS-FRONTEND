...
import { FiRefreshCw, FiUserCheck, FiUserX } from 'react-icons/fi';
...
export default function AdminUsers() {
  ...
  const roleBadge = (role) => {
    const map = { patient: 'bg-blue-500/20 text-blue-400', emt: 'bg-green-500/20 text-green-400', admin: 'bg-purple-500/20 text-purple-400', superadmin: 'bg-red-500/20 text-red-400' };
    ...
    <AdminLayout title="Users">
      <div className="ems-card mb-5">
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
          <select value={filters.role} onChange={e => { setFilters(f => ({ ...f, role: e.target.value })); setPage(1); }} className="ems-input text-sm">
            <option value="">All Roles</option>
            {['patient','emt','admin','superadmin'].map(r => <option key={r} value={r}>{r}</option>)}
          </select>
          <input placeholder="Search name, email, phone..."
            value={filters.search}
            onChange={e => { setFilters(f => ({ ...f, search: e.target.value })); setPage(1); }}
            className="ems-input text-sm" />
          <button onClick={fetchUsers} className="flex items-center justify-center gap-2 px-4 py-2 rounded-xl border border-ems-border text-ems-muted hover:text-white transition-colors">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-ems-border">
                {users.map(u => (
                  <tr key={u._id} className="hover:bg-ems-dark transition-colors">
                    ...
                    <td className="py-3 pr-4"><span className={`status-badge ${roleBadge(u.role)}`}>{u.role}</span></td>
                    ...
                    <td className="py-3">
                      <button onClick={() => toggleActive(u._id, u.isActive)}
                        className={`p-1.5 rounded-lg transition-colors ${u.isActive ? 'text-red-400 hover:bg-red-500/10' : 'text-green-400 hover:bg-green-500/10'}`}
            ...
            {users.length === 0 && <p className="text-center text-ems-muted py-10">No users found</p>}
          </div>
        )}
        {total > 20 && (
          <div className="flex items-center justify-between mt-4 pt-4 border-t border-ems-border">
            ...
            <div className="flex gap-2">
              <button disabled={page === 1} onClick={() => setPage(p => p-1)} className="px-3 py-1.5 rounded-lg border border-ems-border text-ems-muted text-xs hover:text-white disabled:opacity-40">Prev</button>
              <button disabled={page * 20 >= total} onClick={() => setPage(p => p+1)} className="px-3 py-1.5 rounded-lg border border-ems-border text-ems-muted text-xs hover:text-white disabled:opacity-40">Next</button>
            </div>
          </div>
        )}
      </div>
